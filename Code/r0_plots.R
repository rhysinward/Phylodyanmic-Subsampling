...
data <- read.csv ('JS_r0.csv',header = TRUE)
data <- select(data,-4)
hk <- as.data.frame(filter (data, Country == 'Hk'))
hk$Strategy <- factor(hk$Strategy,levels = c("Unsampled", "Proportional", "Uniform", "Reciprocal-proportional "))
br <- filter (data, Country == 'Br')
br$Strategy <- factor(br$Strategy,levels = c("Unsampled", "Proportional", "Uniform", "Reciprocal-proportional "))

#plot

hk_plot <- ggplot(hk, aes(x = Strategy,y = JS, fill = Strategy )) +  
...
br_plot <- ggplot(br, aes(x = Strategy,y = JS, fill = Strategy )) +  
  geom_bar(stat = "identity" ) +
  scale_fill_viridis(discrete = TRUE) +
  theme(legend.position="none") +
  theme_bw() +
...
library(ggpubr)
ggarrange(
  HongKong, hk_plot, Brazil, br_plot, labels = c("A", "B", 'C','D'),
  common.legend = FALSE
)
